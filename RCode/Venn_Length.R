...
threshold=33
blast_hits = as.vector(unlist(dbGetQuery(session, paste("select ko_id from aligners_length where tag=\"BD\" and aln_length>",threshold," order by ko_id ASC",sep=""))))
last_hits = as.vector(unlist(dbGetQuery(session, paste("select ko_id from aligners_length where tag=\"LAD\" and aln_length>",threshold," order by ko_id ASC",sep=""))))
diamond_hits = as.vector(unlist(dbGetQuery(session, paste("select ko_id from aligners_length where tag=\"DSD\" and aln_length>",threshold," order by ko_id ASC",sep=""))))
lambda_hits = as.vector(unlist(dbGetQuery(session, paste("select ko_id from aligners_length where tag=\"LSD\" and aln_length>",threshold," order by ko_id ASC",sep=""))))
pauda_hits = as.vector(unlist(dbGetQuery(session, paste("select ko_id from aligners_length where tag=\"PSD\" and aln_length>",threshold," order by ko_id ASC",sep=""))))
...
png(filename = "venn_ko_length_33.png");
grid.draw(venn.plot);
dev.off();
