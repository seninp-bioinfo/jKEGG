...
blast_hits = as.vector(unlist(dbGetQuery(session, "select distinct(ko_id) from aligners_tops_similarity where tag=\"BD\" order by hit_id ASC")))
last_hits = as.vector(unlist(dbGetQuery(session, "select distinct(ko_id) from aligners_tops_similarity where tag=\"LAD\" order by hit_id ASC")))
diamond_hits = as.vector(unlist(dbGetQuery(session, "select distinct(ko_id) from aligners_tops_similarity where tag=\"DSD\" order by hit_id ASC")))
lambda_hits = as.vector(unlist(dbGetQuery(session, "select distinct(ko_id) from aligners_tops_similarity where tag=\"LSD\" order by hit_id ASC")))
pauda_hits = as.vector(unlist(dbGetQuery(session, "select distinct(ko_id) from aligners_tops_similarity where tag=\"PSD\" order by hit_id ASC")))
