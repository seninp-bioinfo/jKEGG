...
last_hits = dbGetQuery(session, "select org_id, count(*) cnt from aligners_score where tag=\"LAD\" group by org_id order by cnt DESC limit 10;")
org_ids=as.character(last_hits$org_id)
seq=paste("\"",paste(org_ids,collapse="\", \""),"\"",sep="")
blast_hits = dbGetQuery(session, paste("select org_id, count(*) cnt from aligners_score where tag=\"BD\" and evalue<0.01 and org_id in (",
...
#  seq,") group by org_id;",sep=""))
diamond_hits = dbGetQuery(session, paste("select org_id, count(*) cnt from aligners_score where tag=\"DSD\" and org_id in (",
  seq,") group by org_id;",sep=""))
lambda_hits = dbGetQuery(session, paste("select org_id, count(*) cnt from aligners_score where tag=\"LSD\" and org_id in (",
  seq,") group by org_id;",sep=""))
pauda_hits = dbGetQuery(session, paste("select org_id, count(*) cnt from aligners_score where tag=\"PSD\" and org_id in (",
  seq,") group by org_id;",sep=""))
...
p=ggplot(df,aes(x=org_id,y=value,fill=variable)) + geom_bar(position="dodge") + 
  ...
  ggtitle("Top 10 most abundant organisms compared by aligner, DNA")
p

ggsave(arrangeGrob(p, ncol=1), width=180, height=180, units="mm",
       file="abundance-DNA-ORG-score-all.png", dpi=100)
