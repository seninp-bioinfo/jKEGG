...
last_hits$rank=1:10
blast_hits = dbGetQuery(session, "select org_id, count(*) cnt from aligners_score where tag=\"BM\" and evalue<0.01  group by org_id order by cnt DESC limit 10;")
blast_hits$rank=1:10
diamond_hits = dbGetQuery(session, "select org_id, count(*) cnt from aligners_score where tag=\"DSM\"  group by org_id order by cnt DESC limit 10;")
diamond_hits$rank=1:10
lambda_hits = dbGetQuery(session, "select org_id, count(*) cnt from aligners_score where tag=\"LSM\" group by org_id order by cnt DESC limit 10;")
lambda_hits$rank=1:10
pauda_hits = dbGetQuery(session, "select org_id, count(*) cnt from aligners_score where tag=\"PSM\" group by org_id order by cnt DESC limit 10;")
...
names(dd)=c("org_id","blast.abd","blast.rank","last.abd","last.rank","diamond.abd","diamond.rank",
...
png(filename = "table_expression_MRNA_ORG.png", width=900, height=400, pointsize = 18);
grid.draw(g1)
dev.off();
